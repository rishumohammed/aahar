...
export function HotelCard({ hotel, className }: HotelCardProps) {
  const defaultPlaceholder = "https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=800";
  const displayImage = getImageUrl(hotel.image) || defaultPlaceholder;
  const rawRating = Number((hotel as any).rating || (hotel as any).googleRating);
      ...
      <div className="p-6 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <span className="text-[9px] font-bold text-aahar-rose uppercase tracking-[0.15em] bg-aahar-rose/5 px-3 py-1.5 rounded-lg border border-aahar-rose/10">
            {(hotel as any).propertyType ? (hotel as any).propertyType.replace(/_/g, " ") : "Resort"}
          </span>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-medium text-aahar-body/60 uppercase tracking-widest line-clamp-1">
            ...
            {hotel.location || (hotel as any).area || (hotel as any).city || "Kerala, India"}
          </div>
        </div>
      </div>
    </Link>
