...
import { Star, MapPin, ShieldCheck, ArrowRight, Bed } from "lucide-react";
...
export function HotelRowCard({ hotel }: HotelRowCardProps) {
  ...
  return (
    <Link 
      ...
      className="group block relative overflow-hidden rounded-[2.5rem] bg-white border-2 border-aahar-border transition-all hover:border-aahar-rose hover:shadow-2xl hover:shadow-aahar-rose/5"
    >
      <div className="flex flex-col md:flex-row p-4 md:p-6 gap-6 md:gap-10">
        <div className="relative w-full md:w-80 aspect-[4/3] md:aspect-square shrink-0 overflow-hidden rounded-[2rem] bg-aahar-wash">
          ...
          <div className="absolute top-4 left-4">
             <div className="bg-white/90 backdrop-blur-md text-aahar-rose flex items-center gap-2 px-4 py-2 rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl">
                <Bed className="h-4 w-4" />
                {hotel.starRating || 5} Stars
             </div>
          </div>
        </div>
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  <span className="text-[10px] font-bold text-aahar-rose uppercase tracking-[0.2em]">{hotel.propertyType || "Resort"}</span>
                  <div className="w-1 h-1 rounded-full bg-aahar-body/20" />
                  <div className="flex items-center gap-1.5">
                    <Star className="h-4 w-4 fill-amber-400 text-amber-400" />
                    <span className="text-xs font-bold text-aahar-dark">{hotel.rating || 4.8}</span>
                  </div>
                </div>
