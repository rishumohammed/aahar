...
"flex items-center gap-4 px-6 py-3 text-sm font-medium transition-colors rounded-r-full",
 ...
 <input 
 type="text"
 placeholder="Search enquiries, guests..."
 ...
 {user?.name ? user.name.charAt(0).toUpperCase() :"HM"}
 </div>
 <div className="hidden sm:block text-left text-sm max-w-[120px]">
 <p className="font-semibold truncate leading-tight">{user?.name ||"Hotel Manager"}</p>
 <p className="text-[10px] text-white/80 uppercase tracking-wider truncate leading-tight">
 {user?.role ==="super_admin"?"Super Admin":"Hotel Partner"}
 </p>
 </div>
 ...
 <p className="font-semibold">{user?.name ||"Hotel Manager"}</p>
 <p className="text-sm text-slate-500">{user?.email ||"manager@example.com"}</p>
 </div>
 <div className="p-2">
 <button 
 onClick={() => {
